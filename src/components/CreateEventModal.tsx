  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (associationLevel === 'VENUE' && selectedVenue) {
        ...
        // Fetch venue details to get parent hierarchy
        const venueResponse = await fetch(`/api/venues/${selectedVenue}`);
        if (venueResponse.ok) {
          const venue = await venueResponse.json();
          // Venues may be associated with sublocations or locations
          // We'll need to query the junction tables to get parent IDs
          // For now, just set the venueId
        }
